    ...
    <li className="mb-2 col-span-2 bg-tertiary rounded-lg shadow relative group divide-y divide-gray-200">
      <div className="flex flex-row justify-between group-hover:shadow-md">
        <Link
          ...
          key={`${text.id} ${text.body.slice(0, 7)}`}
          to={`/texts/${text.id}`}
        >
          <div
          ...
          >
            ...
            <div className="flex-1 truncate">
              <div className="flex items-center space-x-3">
                <h2 className="text-tertiary text-xl font-medium truncate">
                  {text.title}
                </h2>
              </div>
              <p className="mt-1 text-gray-500 text-sm truncate">{`${text.body.slice(
                0,
                97
              )}...`}</p>
            </div>
...
const Stats = function() {
  ...
  const getWordCount = function() {
    ...
    setFamiliarWords(
      userwords.filter((word) => word.status === 'familiar').length
    );
    setLearnedWords(
      userwords.filter((word) => word.status === 'learned').length
    );
  };
  ...
  return (
    <div className="md:col-span-1 hidden md:flex flex-col gap-6 bg-tertiary rounded-lg shadow p-6">
      <div className="flex gap-2 items-center">
        ...
        <div>
          <h2 className="text-lg font-medium leading-6 text-gray-500">
            Learning Words:
          </h2>
            ...
            <div className="pb-5 border-b gap-4 border-gray-200 dark:border-gray-700 flex justify-between items-start">
              {user && textList.length === 0 ? (
                <h2 className="text-lg leading-6 font-medium text-tertiary">
                  {user.verified === true
                    ? `You have no texts in ${
                        names[user.learnLanguageId]
                      }, please add a text to begin.`
                    : ''}
                </h2>
              ) : (
                <h2 className="text-lg leading-6 font-medium text-tertiary pr-16">
                  Texts
                </h2>
              )}
              {user && user.verified === true ? (
                ...
                <span>
                  You need to verify your email address to add texts.{' '}
                  <Link
        ...
        <div className="grid md:grid-cols-[1fr,1fr,320px] gap-3 grid-cols-1">
          <ul className="md:col-start-0 md:col-span-2">
            {textList && textList.length > 0
              ? textList.map((text) => (
                  <IndividualText
                    key={`${text.id} ${text.body.slice(1, 8)}`}
