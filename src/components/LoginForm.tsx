...
export default function LoginForm({ from }: { from: string }) {
  ...
  return (
    <div className="min-h-full flex items-center gap-6 justify-center py-12 px-6 sm:px-8 lg:px-10">
      <div className="max-w-7x sm:rounded-md mx-auto py-8 px-4 sm:py-6 sm:px-6 lg:px-8">
        <div>
          <img
        ...
        <form key={1} className='flex flex-col gap-4' onSubmit={handleSubmit(userInfo)}>
          ...
          <div>
            ...
            <input {...register('password', { required: true })}
              className="input appearance-none rounded-md relative block bg-four dark:border-transparent w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-tertiary focus:outline-none focus:ring-sky-500 focus:border-sky-500 focus:z-10 sm:text-sm"
