  ...
  const extractTextFromURL = async function(event: FormEvent<HTMLFormElement>) {
    ...
    if (newTextExtractionURL.startsWith('http')) {
      ...
      if (textObject?.content) {
        ...
        setWarningMessage('Text extraction failed. Please try another link or copy the text manually');
          ...
          <form action="#" method="POST" onSubmit={(event) => {
            ...
            <div className="sm:rounded-md sm:overflow-hidden">
              ...
              <div className="px-4 py-3 flex justify-end bg-secondary text-right sm:px-6">
                <button
                  type="submit"
                  className={`inline-flex mx-2 ${newTextBody && newTextTitle ? 'bg-sky-600 text-white border-0 focus:ring-sky-500 hover:bg-sky-500' : 'bg-gray-300 dark:bg-gray-600 text-gray-400 pointer-events-none'} justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white focus:outline-none focus:ring-2 focus:ring-offset-2`}
                >
                  Save
                </button>
                ...
                <div className="">
                  <div className="col-span-3 sm:col-span-2">
                    ...
                    <div className="mt-1 flex rounded-md shadow-sm">
                      ...
                      <button
                        type="submit"
                        className={`inline-flex focus:ring-0 justify-center py-2 px-4 border border-l-0 border-gray-300 dark:border-transparent shadow-sm text-sm font-medium rounded-r-md text-white ${newTextExtractionURL ? 'bg-sky-600 text-white border-1 border-sky-600 focus:ring-sky-600 hover:bg-sky-600' : 'bg-gray-300 dark:bg-gray-600 text-gray-400 pointer-events-none'} focus:outline-none focus:ring-2 focus:ring-offset-2 `}
                      >
                        Extract
                      </button>
                    </div>
                    {showURLWarning && <p className='text-sm text-red-500'>{warningMessage}</p>}
                  </div>
                </div>
              </div>
            </div>
